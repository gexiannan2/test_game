                                                               ...
                                                               const mongoc_structured_log_opts_t *opts)
{
   ...
   if (!mcommon_json_append_bson_values(&append, cmd->command, BSON_JSON_MODE_RELAXED, true, BSON_MAX_RECURSION - 1u)) {
   ...
   for (size_t i = 0; i < cmd->payloads_count; i++) {
      ...
      if (!mcommon_json_append_separator(&append) ||
          !mcommon_json_append_key(&append, field_name, strlen(field_name)) || !mcommon_string_append(&append, "[ ")) {
         goto done;
      }
