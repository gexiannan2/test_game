...
不能把“只传速度和方向”理解成能够完全防止作弊。准确的边界是：

> 客户端只传操作意图，服务器自行计算速度和位置，可以防住主要的瞬移、加速和穿墙等位置类作弊，但不能自动防住所有外挂。
...
```cpp
const Vec2 direction = NormalizeAndClamp(input.direction);
const float analog = std::clamp(input.analog_magnitude, 0.0f, 1.0f);
const float max_speed = GetAuthoritativeMoveSpeed(player);
const Vec3 desired_velocity = ToWorldDirection(direction) * max_speed * analog;
```
...
velocity = MoveTowards(
    current_velocity,
    desired_velocity,
    max_acceleration * fixed_dt);
```

最终将 `velocity` 交给 Jolt CharacterVirtual，由服务端碰撞世界计算权威 `x/y/z`。
...
即使服务器自己积分位置，也可能按照作弊速度推进角色。因此协议中的客户端 `velocity` 最多只能用于预测误差统计：

```cpp
// 只能用于 telemetry、误差分析或风险检测。
const Vec3 reported_client_velocity = request.velocity();

// 权威移动只能使用服务端配置与状态计算出的速度。
const float authoritative_speed = GetAuthoritativeMoveSpeed(player);
```

客户端预测坐标和速度可以作为 telemetry 上传，用于测量误差和发现异常，但永远不能成为权威状态。

### 13.4 输入校验

只传方向仍然可能被滥用，服务端至少实施以下校验：

| 客户端行为 | 服务端处理 |
|------------|------------|
| 方向向量传入 `(100, 100)` | 拒绝非有限值，并把有效方向长度夹到 `[0, 1]` |
| 每秒发送数千个输入包 | 单连接和单实体限流，每 tick 只消费有限输入 |
| 修改客户端时间加快模拟 | 服务端只使用自己的固定 `fixed_dt` |
| 重复发送跳跃 | 检查接地、空中状态、跳跃次数和冷却 |
| 眩晕、死亡时继续移动 | 权威状态机忽略输入或明确拒绝 |
| 伪造跑步速度 | 实际速度只从服务端属性和配置产生 |
| 修改本地 `x/y/z` | 服务端不采用该坐标，其他玩家也看不到假位置 |
| 删除客户端碰撞实现穿墙 | Jolt 使用服务端地图和碰撞体计算最终位置 |
| 重放旧输入包 | 检查 `input_seq`、session epoch 和接受窗口 |
| 提交未来输入 | 检查 future window，超限输入丢弃并计数 |
| 断网后持续移动 | 输入超时后自动切换为中性输入并减速停止 |
| 构造非法按键组合 | 校验 bitmask 和移动状态转换 |

传送、复活和跨图只能由服务端事件触发。连续纠错、输入洪泛、非法状态和时间漂移应形成风险评分；风险评分用于观测、限流和人工规则，不因一次网络抖动直接封禁。
...
- 在眩晕、死亡或硬直状态下移动；
- 伪造客户端时间加快移动；
- 让未经服务器确认的预测位置参与 AOI、战斗或持久化。

例如外挂把本地人物直接修改到 100 米以外：

```text
客户端画面：暂时显示在 100 米外
服务器位置：仍然保持权威位置
其他玩家：只收到服务器权威位置
下一次 owner 快照：作弊客户端被纠正回服务器位置
```
...
这些问题还需要 AOI 信息最小化、行为检测、技能权威判定、客户端完整性保护和专门的反作弊系统。输入权威不是完整反作弊系统，而是生产级移动安全的基础。
