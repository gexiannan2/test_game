...
namespace mongo
{
        ...
        bool background)
    {
        ValidateCollection(collection);
        const std::string collectionName{collection};
        auto request = AcquireRequest("create_index");

        bsoncxx::builder::basic::document index_doc;
        index_doc.append(bsoncxx::builder::basic::kvp("key", keys));

        if (!name.empty())
        {
            index_doc.append(bsoncxx::builder::basic::kvp(
                "name", bsoncxx::types::b_string{std::string{name}}));
        }
        if (unique)
        {
            index_doc.append(bsoncxx::builder::basic::kvp("unique", true));
        }
        if (sparse)
        {
            index_doc.append(bsoncxx::builder::basic::kvp("sparse", true));
        }
        if (background)
        {
            index_doc.append(bsoncxx::builder::basic::kvp("background", true));
        }

        Execute("create_index", [&]() {
            request.Client()[config_.database][collectionName]
                .create_index(index_doc.view());
        });
        request.Succeed();
    }
