...
void SessionService::OnHandshake(const EntityPtr& entity,
...
void SessionService::OnHeartbeat(const EntityPtr& entity) {
  auto* conn = entity->GetComponent<ConnectionComponent>();
  if (conn) {
    conn->last_heartbeat_sec_ = static_cast<uint64_t>(std::time(nullptr));
  }
...
void SessionService::OnReconnect(const EntityPtr& entity,
                                    uint64_t session_id) {
...
void SessionService::OnKickoff(const EntityPtr& entity, uint64_t role_id,
                                  uint32_t code_id,
