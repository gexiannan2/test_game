...
struct SortRelationEntityFunc3D {
  bool operator()(const SortEntityData3D& a, const SortEntityData3D& b) const {
    return a.cvt.LengthSq() < b.cvt.LengthSq();
  }
};
...
bool MoveComponent::UpdateMovePos(WorldSystem* world, Entity* entity,
  ...
  while (time_left > 0.f && !IsEmptyMoveTarget()) {
    ...
    cur_pos = cur_pos + v.Normalized() * step;
    time_left = 0.f;
    break;
  }
    ...
    entity->SetVelocity(direction.Normalized() * speed, EntityPropertyType::kMove);
    world->MoveEntity(entity->SharedHandle(), cur_pos, EntityPropertyType::kMove);
  }
...
void MoveComponent::CheckPreObstacle(WorldSystem* world, Entity* entity) {
  ...
  if (v.LengthSq() > kCheckObstacleDistance * kCheckObstacleDistance) {
    check_pos = cur_pos + v.Normalized() * kCheckObstacleDistance;
  }

  if (pre_check_obstacle_valid_) {
    const Vector3D dest = pre_check_obstacle_pos_ - check_pos;
    if (dest.LengthSq() <= kCheckObstacleDistance * kCheckObstacleDistance) {
      return;
    }
  }
