...
{
    LOG_INFO << "PostOne: role_id=" << roleId << " start";
    ...
    // 诊断日志：SyncToData 后确认坐标已从 TransformComponent 同步到 pdc->data
    {
        auto* tfm_diag = entity->GetComponent<TransformComponent>();
        LOG_INFO << "PostOne: role_id=" << roleId
                 << " after SyncToData"
                 << " tfm.pos=(" << (tfm_diag ? tfm_diag->pos_.GetX() : 0.0f) << ","
                 << (tfm_diag ? tfm_diag->pos_.GetY() : 0.0f) << ","
                 << (tfm_diag ? tfm_diag->pos_.GetZ() : 0.0f) << ")"
                 << " data.pos=(" << pdc->data.base().pos().x() << ","
                 << pdc->data.base().pos().y() << ","
                 << pdc->data.base().pos().z() << ")"
                 << " dirty=" << (pdc->IsDirty() ? 1 : 0);
    }
    ...
    if (!snapshot)
    {
        LOG_WARN << "PostOne: role_id=" << roleId << " SerializeToDB returned nullopt";
        return;  // 快照组装失败，跳过
    }

    LOG_INFO << "PostOne: role_id=" << roleId
             << " snapshot.pos=(" << snapshot->base().pos().x() << ","
             << snapshot->base().pos().y() << ","
             << snapshot->base().pos().z() << ")"
             << " sequence=" << sequence;
    ...
    pdc->ClearDirty();
    LOG_INFO << "PostOne: role_id=" << roleId << " ClearDirty done";
    ...
    auto completion =
            ...
            bool success, std::int64_t, std::exception_ptr ep)
        {
                         ...
                         mongo_worker_thread_id, expected_eventloop_thread_id]()
            {
                ...
                if (success)
                {
                    LOG_INFO << "PostOne fire: role_id=" << roleId
                             << " sequence=" << sequence << " persist success";
    ...
    // 从 AccountComponent 取 account_id，传给 PostSave 写入 players 顶层 account_id 字段。
    // account_id == 0 表示尚未从 mongo 拿到（首次登录异步回调未完成），SaveSnapshot 不写该字段。
    std::int64_t account_id = 0;
    auto* acc = entity->GetComponent<AccountComponent>();
    if (acc)
    {
        account_id = acc->account_id_;
    }

    if (!storage_->PostSave(std::move(*snapshot), sequence, account_id, std::move(completion)))
    {
    ...
    {
        LOG_INFO << "PostOne: role_id=" << roleId
                 << " PostSave posted, sequence=" << sequence;
    }
}
...
void PlayerPersistSystem::TickPersist(std::size_t maxBatch)
{
    ...
    size_t in_map_count = 0;
    for (const auto& [roleId, entity] : all)
    {
        // 只扫描在线（在图中）的玩家。断线 entity 的落地由 FlushPlayer 负责，
        // 避免断线 entity 因落地失败反复重试导致死循环（mongo 不可达时尤其明显）。
        // 如果 FlushPlayer 落地失败，dirty 被重标，但 tick 不再扫断线 entity，
        // 脏数据等下次上线时 FlushPlayer 再落地。
        if (!entity->IsInMap())
        {
            continue;
        }
    ...
    LOG_INFO << "TickPersist: total_players=" << all.size()
             << " in_map=" << in_map_count
             << " maxBatch=" << maxBatch
...
void PlayerPersistSystem::FlushPlayer(uint64_t roleId)
{
    ...
    if (pdc == nullptr)
    {
        // 未进图加载存档的玩家（kConnected/kHandshaked/kLoggedIn 阶段）跳过。
        LOG_INFO << "FlushPlayer skipped: no PlayerDataComponent (entity not in game yet), role_id=" << roleId;
        return;
    }
    PostOne(roleId);
}
