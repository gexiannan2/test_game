...
bool MapSystem::IsRelationEntity(const Vector3D& start_pos,
    ...
    if (cv.LengthSq() <= r2 || tv.LengthSq() <= r2) {
        return false;
    }

    const Vector3D vt = cv - v * (v.Dot(cv) / v.LengthSq());
    if (vt.LengthSq() >= r2) {
        return false;
    }
...
PathCalcResult MapSystem::CalcMoveSpherePath(const Vector3D& start_pos,
    ...
    const float cvl = cv.Length();
    const float tvl = tv.Length();
    if (tvl <= 1e-6f) {
        return PathCalcResult::kNone;
    }
    ...
    if (tvl <= radius) {
        const Vector3D temp = center_pos - cv.Normalized() * radius;
    ...
    if (axis.IsNearZero()) {
        ...
        if (cv.Cross(axis).IsNearZero()) {
            axis = Vector3D(0.f, 1.f, 0.f);
        }
    }
    axis = axis.Normalized();

    Vector3D tangent = RotateAroundAxis(tv, axis, angle).Normalized() * radius;
    const Vector3D proj = -(cv - v * (v.Dot(cv) / v.LengthSq()));
    if (tangent.Dot(proj) < 0.f) {
        tangent = RotateAroundAxis(tv, axis, -angle).Normalized() * radius;
    }
    Vector3D inter_pos_1 = center_pos - cv.Normalized() * radius;
