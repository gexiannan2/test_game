...
GAME_TEST(AoiAppearDisappear, LeaveAoiRangeDisappears) {
  ...
  tw->world->MoveEntity(b, JPH::Vec3(300.0f, 0.0f, 300.0f), EntityPropertyType::kMove);
...
GAME_TEST(AoiAppearDisappear, FullCycleMultipleChurns) {
  ...
  for (int round = 1; round <= 3; ++round) {
    ...
    // B 离开
    tw->world->MoveEntity(b, JPH::Vec3(200.0f, 0.0f, 200.0f), EntityPropertyType::kMove);
    ...
    // B 回来
    tw->world->MoveEntity(b, JPH::Vec3(1.5f, 0.0f, 1.5f), EntityPropertyType::kMove);
    EXPECT_TRUE(tw->SawAppear(1, 2));
    EXPECT_TRUE(tw->SawAppear(2, 1));
...
// ======================================================================
// 用例 5: 相邻 AOI 格（±1 邻域）仍互相可见
// ======================================================================
GAME_TEST(AoiAppearDisappear, AdjacentCellsSeeEachOther) {
  std::cout << std::endl;
  LOG_TEST() << ">>> 用例: A cell(0,0), B cell(1,0) 相邻格互见 <<<" << std::endl;

  auto tw = TestWorld::Create();

  // cell_size=10: A@(0,0)→cell0, B@(11,0)→cell1；kNeighborhoodRadius=1 覆盖邻格
  auto a = tw->SpawnPlayer(1, 0.0f, 0.0f);
  auto b = tw->SpawnPlayer(2, 11.0f, 0.0f);
  tw->Dump("Spawn: A(0,0) B(11,0) adjacent cells");

  EXPECT_TRUE(tw->SawAppear(1, 2));
  EXPECT_TRUE(tw->SawAppear(2, 1));

  auto vis_a = tw->world->GetVisibleEntities(1);
  auto vis_b = tw->world->GetVisibleEntities(2);
  EXPECT_TRUE(std::find(vis_a.begin(), vis_a.end(), 2u) != vis_a.end());
  EXPECT_TRUE(std::find(vis_b.begin(), vis_b.end(), 1u) != vis_b.end());

  // 同格挪动不触发 appear；用可见性查询验证仍互见
  tw->stats.clear();
  tw->world->MoveEntity(b, JPH::Vec3(9.5f, 0.0f, 0.0f), EntityPropertyType::kMove);  // 仍在 cell(0,0)
  tw->Dump("Move: B(9.5,0) same cell — no new appear expected");

  vis_a = tw->world->GetVisibleEntities(1);
  vis_b = tw->world->GetVisibleEntities(2);
  EXPECT_TRUE(std::find(vis_a.begin(), vis_a.end(), 2u) != vis_a.end());
  EXPECT_TRUE(std::find(vis_b.begin(), vis_b.end(), 1u) != vis_b.end());
  EXPECT_FALSE(tw->SawAppear(1, 2));  // 已可见，同格移动不应再推 appear

  LOG_TEST() << "  PASS: radius=1 covers adjacent cell ✓" << std::endl;
}
...
GAME_TEST(AoiAppearDisappear, StressAppearDisappearStats) {
  ...
  for (size_t i = 0; i < players.size(); ++i) {
    float x = static_cast<float>(i) * 0.5f;
    float z = static_cast<float>(i % 5) * 0.5f;
    tw->world->MoveEntity(players[i], JPH::Vec3(x, 0.0f, z), EntityPropertyType::kMove);
  }
