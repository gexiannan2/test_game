...
GAME_TEST(ProductionTest, ConcurrentRoleCreateDeleteIndexIntegrity) {
  ...
  for (int i = 0; i < kN; ++i) {
    ...
    SessionService::OnRoleCreate(entity, rid, "P" + std::to_string(i), 1, 1);
...
GAME_TEST(ProductionTest, MultiAccountUidIsolation) {
  ...
  SessionService::OnRoleCreate(e1, 40001, "A1", 1, 1);
  SessionService::OnRoleCreate(e2, 40002, "B1", 2, 2);
  ...
  bool deny = SessionService::OnRoleLogin(e2, 40001);
  EXPECT_FALSE(deny);
...
GAME_TEST(ProductionTest, RoleLoginRoundTripAfterPartialDelete) {
  ...
  SessionService::OnRoleCreate(entity, 40010, "Keep", 1, 1);
  SessionService::OnRoleCreate(entity, 40011, "Drop", 2, 2);
  SessionService::OnRoleDelete(entity, 40011);

  EXPECT_TRUE(SessionService::OnRoleLogin(entity, 40010));
...
GAME_TEST(ProductionTest, BornPosMoveBackLikeGameServer) {
  ...
  disappear_to_a = 0;
  world->MoveEntity(b, JPH::Vec3(1000.0f, 20.0f, 1000.0f), EntityPropertyType::kMove);
  b->SetMoveState(JPH::Quat::sIdentity(), JPH::Quat::sIdentity(),
                  JPH::Vec3(1, 0, 0), EntityPropertyType::kMove);
  EXPECT_TRUE(disappear_to_a >= 1);

  appear_to_a = 0;
  world->MoveEntity(b, JPH::Vec3(kBornX, kBornY, kBornZ), EntityPropertyType::kMove);
  b->SetMoveState(JPH::Quat::sIdentity(), JPH::Quat::sIdentity(),
                  JPH::Vec3(1, 0, 0), EntityPropertyType::kMove);
