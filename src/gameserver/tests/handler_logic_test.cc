...
GAME_TEST(HandlerLogicTest, UnregisterKeepsUidAfterRoleDelete) {
  ...
  SessionService::OnRoleCreate(entity, 20001, "HeroA", 1, 1);
  SessionService::OnRoleCreate(entity, 20002, "HeroB", 2, 2);
  ...
  SessionService::OnRoleDelete(entity, 20001);
...
GAME_TEST(HandlerLogicTest, OnRoleLoginRejectsInvalidRoleId) {
  ...
  SessionService::OnRoleCreate(entity, 20003, "Valid", 1, 1);

  bool ok = SessionService::OnRoleLogin(entity, 99999);
  EXPECT_FALSE(ok);
...
GAME_TEST(HandlerLogicTest, OnRoleLoginAcceptsValidRoleId) {
  ...
  SessionService::OnRoleCreate(entity, 20004, "ValidHero", 1, 1);

  bool ok = SessionService::OnRoleLogin(entity, 20004);
  EXPECT_TRUE(ok);
  EXPECT_EQ(static_cast<int>(entity->GetState()),
...
GAME_TEST(HandlerLogicTest, DeleteAllRolesClearsRoleIndexButKeepsUid) {
  ...
  SessionService::OnRoleCreate(entity, 20005, "Only", 1, 1);
  SessionService::OnRoleDelete(entity, 20005);
...
GAME_TEST(HandlerLogicTest, RandomNameReturnsNonEmptyForValidSex) {
  std::string name = SessionService::OnRandomNameReq(1);
  EXPECT_TRUE(!name.empty());
  std::string name2 = SessionService::OnRandomNameReq(2);
...
GAME_TEST(HandlerLogicTest, RoleMarkLastConsistency) {
  ...
  SessionService::OnRoleCreate(entity, 20006, "A", 1, 1);
  SessionService::OnRoleCreate(entity, 20007, "B", 2, 2);
  ...
  bool ok = SessionService::OnRoleLogin(entity, 20006);
  EXPECT_TRUE(ok);
  last_count = 0;
  for (const auto& r : role->all_roles_) {
    if (r.is_last) ++last_count;
...
GAME_TEST(HandlerLogicTest, UnregisterStressManyRoles) {
  ...
  for (int i = 0; i < kRoles; ++i) {
    SessionService::OnRoleCreate(entity, 30000u + static_cast<uint64_t>(i),
                         "R" + std::to_string(i), 1, 1);
  }
  EXPECT_EQ(PlayerEntitySystem::Instance().FindByUid("uid_stress"), entity);

  for (int i = 0; i < kRoles; ++i) {
    SessionService::OnRoleDelete(entity, 30000u + static_cast<uint64_t>(i));
    EXPECT_EQ(PlayerEntitySystem::Instance().FindByUid("uid_stress"), entity);
