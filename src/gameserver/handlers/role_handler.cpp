                          ...
                          const std::shared_ptr<::google::protobuf::Message>& req)
{
    ...
    if (msg_id == proto_id("cli_role_list_req"))
    {
            ...
            req_ptr->session_id() != 0)
        {
                ...
                cached_acc->session_id_ == req_ptr->session_id())
            {
                ...
                opts.leave_map_after_bind = true;
                opts.clear_data_loading = true;
                opts.apply_state_after = true;
                opts.state_after = Entity::State::kLoggedIn;
                RebindCachedEntity(server_, cached, cur_entity, conn,
                                   server::kKickoffReplaceAccount,
                                   "role_list_session_restore", opts);
                cur_entity = cached;
            }
            else
            {
        ...
        SessionService::OnRoleListReq(cur_entity);
    ...
    else if (msg_id == proto_id("cli_random_name_req"))
    {
        ...
        std::string name = SessionService::OnRandomNameReq(req_ptr->sex());
        ::cli_random_name_res res;
        if (!name.empty())
        {
    ...
    else if (msg_id == proto_id("cli_role_create_req"))
    {
        ...
        SessionService::OnRoleCreate(cur_entity, new_role_id, req_ptr->role_info().name(),
    ...
    else if (msg_id == proto_id("cli_role_delete_req"))
    {
        ...
        SessionService::OnRoleDelete(cur_entity, req_ptr->role_id());
        // 删除 mongo 存档（异步，不阻塞回包）。
        // mongo 未启用时 GetPlayerArchive() 返回 nullptr，跳过（行为等价原直接调）。
        if (auto archive = server_->GetPlayerArchive())
        {
            archive->DeleteRoleArchive(req_ptr->role_id());
        }
        ::cli_role_list_res res;
        res.set_err_code(server::kRoleErrOk);
    ...
    else if (msg_id == proto_id("cli_role_login_req"))
    {
        ...
        const auto check = SessionService::CheckRoleLogin(cur_entity, req_ptr->role_id());
        if (check != SessionService::RoleLoginCheck::kOk)
        {
        ...
        if (!SessionService::OnRoleLogin(cur_entity, req_ptr->role_id()))
        {
