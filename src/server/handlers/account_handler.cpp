...
void AccountHandler::Handle(const ::zrpc::TcpConnectionPtr& conn,
  ...
  if (existing && existing != entity) {
    ...
    // 顶号须先离图再踢线，避免 kickoff 时 AOI 仍绑在即将关闭的连接上
    if (existing->IsInMap()) {
      server_->GetWorld()->LeaveMap(existing);
    }

    server_->KickAndShutdownConnection(
        existing, server::kKickoffReplaceAccount, "re_login", conn);

    if (!existing->HasComponent<ConnectionComponent>()) {
      existing->AddComponent<ConnectionComponent>();
    }
    auto* cc = existing->GetComponent<ConnectionComponent>();
    cc->conn_ = conn;
    cc->last_heartbeat_sec_ = static_cast<uint64_t>(std::time(nullptr));
