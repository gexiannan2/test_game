...
void Entity::SetPosition(const JPH::Vec3& position) {
  MutateTransform([&](auto& tfm) { tfm.pos_ = position; }, EntityPropertyType::kMove);
}

void Entity::SetDirection(const JPH::Quat& rotation) {
  MutateTransform([&](auto& tfm) { tfm.rot_ = rotation; }, EntityPropertyType::kMove);
}
...
void Entity::SetMoveRot(const JPH::Quat& move_rot) {
  MutateTransform([&](auto& tfm) { tfm.move_rot_ = move_rot; }, EntityPropertyType::kMove);
}
...
void Entity::SetVelocity(const JPH::Vec3& vel) {
  MutateTransform([&](auto& tfm) { tfm.velocity_ = vel; }, EntityPropertyType::kMove);
}

void Entity::SetMoveState(const JPH::Quat& rot,
                          const JPH::Quat& move_rot,
                          const JPH::Vec3& vel) {
  MutateTransform([&](auto& tfm) {
                      tfm.rot_ = rot;
...
void Entity::TransferGameplayComponentsFrom(const EntityPtr& from) {
  if (!from || from.get() == this) return;

  if (auto* src = from->GetComponent<RoleComponent>()) {
    if (!HasComponent<RoleComponent>()) AddComponent<RoleComponent>();
  ...
  if (auto* src = from->GetComponent<MapComponent>()) {
    if (!HasComponent<MapComponent>()) AddComponent<MapComponent>();
    *GetComponent<MapComponent>() = *src;
  }

  if (auto* src = from->GetComponent<PlayerDataComponent>()) {
    if (!HasComponent<PlayerDataComponent>()) AddComponent<PlayerDataComponent>();
