...
class Entity : public std::enable_shared_from_this<Entity> {
 public:
  ...
  std::shared_ptr<Entity> SharedHandle() { return shared_from_this(); }

  uint64_t GetId() const { return id_; }
  uint64_t GetEntityId() const { return id_; }
  ...
  void SetMoveState(const JPH::Quat& rot,
                    ...
                    const JPH::Vec3& vel);
  void SuppressPersistDirty(bool suppress) { suppress_persist_dirty_ = suppress; }
  bool IsPersistDirtySuppressed() const { return suppress_persist_dirty_; }
  ...
  bool IsInMap() const { return is_in_map_.load(std::memory_order_acquire); }
  void SetInMap(bool in_map) { is_in_map_.store(in_map, std::memory_order_release); }

  // 连接/会话状态机：kConnected → kHandshaked → kLoggedIn → kRoleSelected → kInGame；
  ...
  MoveComponent* Move() { return GetComponent<MoveComponent>(); }
  const MoveComponent* Move() const { return GetComponent<MoveComponent>(); }

  // 脏属性：SetPropertyDirty → AoiSystem 脏队列 → Flush → SerializeDirty。
  // 同一 EntityPropertyType 在 dirty_property_types_ 中只保留一条。
  bool IsDirty() const { return !dirty_property_types_.empty(); }
  ...
  virtual bool IsPlayerEntity() const { return IsPlayer(); }
  virtual bool NeedsAoiWatcher() const { return IsPlayer(); }
  ...
  void MutateTransform(Fn&& fn, EntityPropertyType type) {
    ...
    if (!tfm) return;
    fn(*tfm);
    if (!suppress_persist_dirty_) OnTransformChanged(type);
  }
  ...
  void SetWorld(WorldSystem* world) { world_ = world; }
  WorldSystem* GetWorld() const { return world_; }
 ...
 private:
  ...
  // 加载/初始化期间屏蔽 OnTransformChanged 触发的持久化标脏。
  bool suppress_persist_dirty_ = false;
};
