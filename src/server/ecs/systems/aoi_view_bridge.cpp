...
void AoiViewBridge::Install() {
    ...
    world_->Map().SetEnterMapCallback(
        [this](const EntityPtr& entity) { OnEntityEnterMap(entity); });
}
...
void AoiViewBridge::OnEntityEnterMap(const EntityPtr& entity) {
    ...
    if (enter_map.SerializeToString(&body) && !body.empty()) {
        if (send_fn_) send_fn_(entity->GetId(), proto_id("cli_3d_enter_map_ntf"), body);
    } else {
