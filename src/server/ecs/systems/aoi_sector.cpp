...
// 收集所有 receiver id（排除 subject 自身），供 per-subject 广播使用。
...
void EntityMonitor::NotifyAppearToReceiver(
    ...
    for (uint64_t sid : subject_ids) {
        // 不跳过自身：玩家进图时也收到自己的 appear（桥接层设 is_self=true）
...
void AoiSector::OnSubjectLeaveMap(const EntityPtr& subject) {
    ...
    if (AoiCell* cell = CellAtPosition(subject->GetPosition())) {
        cell->OnSubjectLeave(subject);
    }
    ForEachCell([sid](AoiCell& cell) {
        cell.Monitor().UnmonitorEntity(sid, false);
    });
