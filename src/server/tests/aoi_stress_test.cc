...
GAME_TEST(AoiStressTest, TwoThousandMoveAndDirtySync) {
    ...
    for (int step = 0; step < MoveSteps; ++step) {
        for (auto& e : players) {
            ...
            sw.world->MoveEntity(e, JPH::Vec3(nx, 0.0f, nz));
            // 随机标脏
            if (rng.Next() % 3 == 0) {
...
GAME_TEST(AoiStressTest, MoveEntityBeforeEnterMapNoCrash) {
    ...
    sw.world->MoveEntity(e, JPH::Vec3(100.0f, 0.0f, 100.0f));
    sw.world->Tick();
...
GAME_TEST(AoiStressTest, NullEntityNoCrash) {
    ...
    sw.world->MoveEntity(null_e, JPH::Vec3(1.0f, 0.0f, 1.0f));
    sw.world->Tick();
    EXPECT_EQ(sw.world->GetEntityCount(), 0u);
}
...
GAME_TEST(AoiStressTest, MassCrossSwapPositions) {
    ...
    for (int i = 0; i < N / 2; ++i) {
        if (players[i] && players[i + N / 2]) {
            sw.world->MoveEntity(players[i], JPH::Vec3(500.0f, 0.0f, static_cast<float>(i * kGrid)));
