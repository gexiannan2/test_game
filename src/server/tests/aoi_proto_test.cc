...
class ProtoCapture {
 public:
    ...
    void Bind(WorldSystem* world) {
        ...
        world->Map().SetEnterMapCallback(
            [this](const EntityPtr& e) { OnEnterMap(e); });
    }
 ...
 private:
    ...
    void OnEnterMap(const EntityPtr& entity) {
        if (!entity) return;
        // 玩家自己收到 cli_3d_enter_map_ntf（与 AoiViewBridge::OnEntityEnterMap 一致）
...
GAME_TEST(AoiProtoTest, DirtySyncBroadcastMove) {
    ...
    tw.world->MoveEntity(a, JPH::Vec3(5.0f, 0.0f, 5.0f));
...
GAME_TEST(AoiProtoTest, DirtySyncBroadcastStopMove) {
    ...
    EXPECT_TRUE(a && b);
    // 给 A 设一个非零速度，模拟移动中
    a->SetVelocity(JPH::Vec3(5.0f, 0.0f, 3.0f));
    tw.capture.Clear();
    tw.world->Tick();

    // 标记 kStopMove：move_handler 停止时 velocity 已清零，这里模拟清零后标脏
    a->SetVelocity(JPH::Vec3::sZero());
...
GAME_TEST(AoiProtoTest, MoveCrossGridDisappearAndReappear) {
    ...
    tw.world->MoveEntity(b, JPH::Vec3(kFarX, 0.0f, kFarZ));
    tw.world->Tick();
    ...
    tw.capture.Clear();
    tw.world->MoveEntity(b, JPH::Vec3(kNearX + 2.0f, 0.0f, kNearZ + 2.0f));
    tw.world->Tick();
...
GAME_TEST(AoiProtoTest, BornPosMoveAwayAndBackReappear) {
    ...
    tw.world->MoveEntity(b, JPH::Vec3(1000.0f, 20.0f, 1000.0f));
    tw.world->Tick();
    const CapturedFrame* df = tw.capture.FirstFrame(a->GetId(), proto_id("cli_3d_aoi_disappears_ntf"));
    EXPECT_TRUE(df != nullptr);

    tw.capture.Clear();
    tw.world->MoveEntity(b, JPH::Vec3(kBornX, kBornY, kBornZ));
    tw.world->Tick();
    const CapturedFrame* af = tw.capture.FirstFrame(a->GetId(), proto_id("cli_3d_aoi_appears_ntf"));
    EXPECT_TRUE(af != nullptr);
    if (af) {
