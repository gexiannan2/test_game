...
GAME_TEST(ProductionFullStackTest, FullAccountRoleFlow) {
  ...
  uint64_t session = System::OnUserLogin(conn_entity, "fullstack_uid", "tok", 1, 70001);
  ...
  System::OnRoleListReq(conn_entity);
  System::OnRoleCreate(conn_entity, 60001, "FullHero", 1, 1);
  EXPECT_TRUE(System::OnRoleLogin(conn_entity, 60001));
  EXPECT_EQ(static_cast<int>(conn_entity->GetState()),
            static_cast<int>(Entity::State::kRoleSelected));

  System::OnRoleCreate(conn_entity, 60002, "AltHero", 2, 2);
  System::OnRoleDelete(conn_entity, 60001);

  EXPECT_EQ(PlayerEntitySystem::Instance().FindByUid("fullstack_uid"), conn_entity);
  EXPECT_EQ(PlayerEntitySystem::Instance().FindByRoleId(60001), nullptr);
  EXPECT_TRUE(System::OnRoleLogin(conn_entity, 60002));
...
GAME_TEST(ProductionFullStackTest, ReLoginSimulationNewConnectionEntity) {
  ...
  System::OnUserLogin(old_conn, "relogin_uid", "t1", 1, 70002);
  System::OnRoleCreate(old_conn, 60003, "Relog", 1, 1);
  EXPECT_TRUE(System::OnRoleLogin(old_conn, 60003));
