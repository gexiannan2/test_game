...
EntityPtr LoginSelectAndEnter(WorldSystem& world, uint64_t entity_id,
  ...
  System::OnRoleCreate(e, role_id, "Hero" + std::to_string(role_id), 1, 1);
  EXPECT_TRUE(System::OnRoleLogin(e, role_id));
...
GAME_TEST(LoginAoiChurnTest, SinglePlayerLoginEnterLeaveReenter) {
  ...
  world->MoveEntity(p, GridCenter(40, 40));
  ...
  for (int r = 0; r < kRounds; ++r) {
    for (int n = 0; n < kPlayers / 2; ++n) {
      ...
      switch (action(rng)) {
        ...
        default:
          if (e->IsInMap()) {
            world->MoveEntity(e, GridCenter(cell(rng), cell(rng)));
          }
          break;
      }
    }
...
GAME_TEST(LoginAoiChurnTest, RapidOnlineOfflineTempEntityCleanup) {
  ...
  System::OnRoleCreate(cached, 83001, "Stable", 1, 1);
  EXPECT_TRUE(System::OnRoleLogin(cached, 83001));
...
GAME_TEST(LoginAoiChurnTest, RoleReplaceTransfersGameplayComponents) {
  ...
  System::OnRoleCreate(old_ent, 84001, "Old", 1, 1);
  EXPECT_TRUE(System::OnRoleLogin(old_ent, 84001));
  ...
  EXPECT_TRUE(System::OnRoleLogin(new_ent, 84001));
